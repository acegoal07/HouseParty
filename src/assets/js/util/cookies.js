 ...
 */
export function setCookie({ name, value, sameSite = "Strict", expires = 1, httpOnly = false }) {
   ...
   document.cookie = `${encodeURIComponent(name)}=${encodeURIComponent(value || "")}; expires=${date.toUTCString()}; SameSite=${sameSite}; Secure; Path=/${httpOnly ? "; HttpOnly" : ""}`;
}
...
*/
export function deleteCookie({ name, sameSite = "Strict" }) {
   if (typeof name !== 'string' || !name) {
      throw new Error('Cookie name must be a non-empty string');
   }
   if (!['Strict', 'Lax', 'None'].includes(sameSite)) {
 ...
 */
export function extendCookie({ name, days, sameSite = "Strict", httpOnly = false }) {
   if (typeof name !== 'string' || !name) {
      throw new Error('Cookie name must be a non-empty string');
   }
